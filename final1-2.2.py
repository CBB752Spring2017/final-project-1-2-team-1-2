...
### Usage:      python final1-2.2.py -i <input folder> -m <mutation file> -t <tissue name>
### Example:    python final1-2.2.py -i GTEx_Analysis_v6p_eQTL -m Z.variantCall.SNPs_filt.vcf -t 'Whole_Blood'
...
import pandas as pd
import os
# os.chdir('./FinalProjects/Project1.2')
...
def runAGEP(ifolder, mutfile, tissue):
    ...
    ### you may import Zimmerome SNPs from previously stored data file
    ...
    for t in tx: ##compare each line in GTEx database if consistent with Zimmerome SNPs
        ...
        if chrom.isdigit():
            chrom = int(chrom)
        if chrom in zids.keys(): ##judgement order is changed: maybe a little better
            pos = line[1]
            if pos.isdigit():
                pos = int(pos)
            if pos in zids[chrom].keys():
                ...
                # if line[2] == zmut.loc[k,'REF'] & line[3] == zmut.loc[k,'ALT']:
                    mids.append(t)
        if not t % 10000: ##a time indicator is added since file is large and thus calculation is slow
            print t
    selected = mids
